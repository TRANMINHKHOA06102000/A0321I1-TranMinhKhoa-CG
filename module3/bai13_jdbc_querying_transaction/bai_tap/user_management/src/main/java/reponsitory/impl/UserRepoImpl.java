...
public class UserRepoImpl implements UserRepo {
    ...
    public User getUserById(int id) {
        ...
        try (Connection connection = this.baseRepo.getConnection();
    ...
    public void addUserTransaction(User user, int[] permision) {
        ...
        ResultSet rs = null;
        try {
            conn = this.baseRepo.getConnection();
    ...
    public void insertUpdateWithoutTransaction() {
        try (Connection conn =this.baseRepo.getConnection();
    ...
    public List<User> selectAllUsersStore() {
        List<User> userList=new ArrayList<>();
        try {
            CallableStatement callableStatement=baseRepo.getConnection().prepareCall("call show_all_users ()");
            ResultSet rs=callableStatement.executeQuery();
            User user=null;
            while (rs.next()){
                user=new User();
    ...
    public boolean updateUserStore(User user) {
        int row=0;
        try {
            CallableStatement callableStatement=baseRepo.getConnection().prepareCall("call edit_user(?,?,?,?)");
            callableStatement.setString(1,user.getName());
            callableStatement.setString(2,user.getEmail());
            callableStatement.setString(3,user.getCountry());
    ...
    public boolean deleteUserStore(int id) {
        int row=0;
        try {
            CallableStatement callableStatement=baseRepo.getConnection().prepareCall("call delete_user(?)");
            callableStatement.setInt(1,id);
            row=callableStatement.executeUpdate();
        } catch (SQLException throwables) {
            throwables.printStackTrace();
